            ...
            <div class="panel panel-default">
                ...
                <form id="editForm" action="/system/module/edit.do" method="post">
                    ...
                    <div class="row data-type" style="margin: 0px">
                        ...
                        <div class="col-md-4 data">
                            <select class="form-control" onchange="document.getElementById('parentName').value=this.options[this.selectedIndex].text" name="parentId" id="parentId">
                        ...
                        <div class="col-md-4 data">
                            <div class="form-group form-inline">
                                <div class="radio"><label><input type="radio" onclick="changeModule(0)"  ${module.ctype==0?'checked':''} name="ctype" value="0">主菜单</label></div>
                                <div class="radio"><label><input type="radio" onclick="changeModule(1)" ${module.ctype==1?'checked':''} name="ctype" value="1">二级菜单</label></div>
                                <div class="radio"><label><input type="radio" onclick="changeModule(2)"  ${module.ctype==2?'checked':''} name="ctype" value="2">按钮</label></div>
                            </div>
                        </div>
                        <script>
                            //执行的方法
                            function changeModule(ctype){
                                if(ctype==0){
                                    $("#parentId").html("<option>请选择</option>");
                                }else{
                                    $.get("/system/module/changeModule.do?ctype="+ctype ,  function(data){
                                        var tempStr = "<option>请选择</option>"; //准备拼接的字符串
                                        $(data).each(function(){
                                            tempStr+="<option value="+this.module_id+">"+this.name+"</option>"
                                        })
                                        $("#parentId").html(tempStr); //覆盖下拉框数据
                                    })
                                }
